package com.qinweizhao.auth.other;
...
//	@Cacheable(value = CacheConstants.CLIENT_DETAILS_KEY, key = "#clientId", unless = "#result == null")
    public RegisteredClient findByClientId(String clientId) {
        ...
        String qwzClientId = "c1";
        String qwzClientSecret = "123";

		RegisteredClient.Builder builder = RegisteredClient.withId(qwzClientId)
				.clientId(qwzClientId)
				.clientSecret(SecurityConstants.NOOP + qwzClientSecret)
				.clientAuthenticationMethod(ClientAuthenticationMethod.CLIENT_SECRET_BASIC);

		// 授权模式
		Optional.ofNullable("password,app,refresh_token,authorization_code,client_credentials")
		...
		// 回调地址
		Optional.ofNullable("www.qinweizhao.com").ifPresent(redirectUri -> Arrays
				.stream(redirectUri.split(StrUtil.COMMA)).filter(StrUtil::isNotBlank).forEach(builder::redirectUri));

		// scope
		Optional.ofNullable("server").ifPresent(
		...
		return builder
				.tokenSettings(TokenSettings.builder().accessTokenFormat(OAuth2TokenFormat.REFERENCE)
						.accessTokenTimeToLive(Duration.ofSeconds(Optional
								.ofNullable(60 * 60 * 12).orElse(accessTokenValiditySeconds)))
						.refreshTokenTimeToLive(
								Duration.ofSeconds(Optional.ofNullable(60 * 60 * 24 * 30)
										.orElse(refreshTokenValiditySeconds)))
						.build())
				.clientSettings(ClientSettings.builder()
						.requireAuthorizationConsent(!BooleanUtil.toBoolean(String.valueOf(true))).build())
				.build();
